...
Get-Mailbox | foreach {
(Get-MailboxPermission -Identity $_.userprincipalname | Where-Object{ ($_.AccessRights -contains "FullAccess") -and ($_.IsInherited -eq $false) -and -not ($_.User -match "NT AUTHORITY") }) | select Identity, AccessRights, User }

#Show all users with "Send As" permissions
Get-Mailbox | foreach {
(Get-RecipientPermission -Identity $_.userprincipalname | Where-Object{ -not (($_.Trustee -match "NT AUTHORITY") -or ($_.Trustee -match "S-1-5-21")) }) | select Identity, trustee }

#Show all users with "Send on behalf" permissions
Get-Mailbox –ResultSize Unlimited | Where-Object{ $_.GrantSendOnBehalfTo -ne $null } | Select UserprincipalName, GrantSendOnBehalfTo
...
Get-EXOMailbox -ResultSize Unlimited | Where-Object { $_.ForwardingAddress -ne $null } | select DisplayName, ForwardingAddress
...
Get-User | Where-Object { $_.Department -eq "Sales" } | Get-Mailbox | Set-Mailbox -ProhibitSendQuota 5GB -ProhibitSendReceiveQuota 5GB -IssueWarningQuota 4GB
...
Get-Mailbox -Filter { Name -Like "NY_MeetingRoom*" } | Where-Object { $_.ResourceType -eq "Room" } | Remove-Mailbox -Confirm:$False -WhatIf

#Create a new distribution group for a list of Room Mailboxes
$Members = Get-Mailbox -Filter { Name -Like "NY_MeetingRoom*" } | Where-Object { $_.ResourceType -eq "Room" } | Select-Object Name -ExpandProperty name
...
Get-CalendarProcessing -Identity  | Where-Object{ $_.ResourceType -eq "Room" } | Format-List
...
Foreach ($mbx in $mbxs) {
    New-Mailbox -Name $mbx.DisplayName -DisplayName $mbx.DisplayName -MicrosoftOnlineServicesID $mbx.UserPrincipalName
} -Password (ConvertTo-SecureString -String 'YourPassword' -AsPlainText -Force) -ResetPasswordOnNextLogon $true -MailboxPlan 'MailboxPlan' 
...
get-mailbox | Where-Object{ $_.name -like "NY*" } | Remove-Mailbox -Confirm:$false -WhatIf
