...
New-Item -Path (Get-ChildItem -Path $file).Directory.FullName -Name '_files' -ItemType Directory -Force
...
do {
    ...
    if ($i -gt $fut) {
        ...
        $current_file = New-Item -Path (Get-ChildItem -Path $file).Directory.FullName -ItemType File  -Name ('_files\_file_{0:d8}.xml' -f $i) -Force
        ...
        $stringBuilder.Append("<northern-lights tenantId=`"ID`">").ToString()
        $stringBuilder.Append("`n").ToString()
        $stringBuilder.Append("`n").ToString()
